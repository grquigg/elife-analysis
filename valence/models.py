...
class BertTokenEmbedder(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        bert_embeddings = outputs["pooler_output"]
...
class BertHSLN(torch.nn.Module):
    ...
    def reinit_output_layer(self, tasks, config):
        ...
        input_dim = self.bert.bert_hidden_size
    ...
    def forward(self, batch, labels=None, output_all_tasks=False):
        ...
        # shape (documents, sentences, hidden_size)
        bert_embeddings = bert_embeddings.view(documents, sentences, self.bert.bert_hidden_size)
        if self.generic_output_layer:
            output = self.crf(bert_embeddings, None, labels)
        else:
            output = self.crf(batch["task"], bert_embeddings, None, labels, output_all_tasks)
